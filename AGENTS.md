...
- **Documentation-only changes do not need a beta push** (user decision 2026-09-24). This covers changes limited to `documentation/`, other Markdown instruction files such as `AGENTS.md`, `.claude/CLAUDE.md` and skill `*.md`, and `tasks/`. Commit them locally on `beta`; they go out with the next code push. Every push redeploys Railway beta (about 10 minutes) and invalidates any running arena precheck or gate, so never push a documentation-only commit during an arena run. Push documentation alone only when the user asks, or when another agent working from a different clone needs it. Report such work as "committed locally, not pushed".
