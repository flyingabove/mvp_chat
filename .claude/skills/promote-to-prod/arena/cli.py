# .claude/skills/promote-to-prod/arena/cli.py
"""Arena command line (entry point: .claude/skills/promote-to-prod/arena_cli.py).
...
Local-only: nothing here runs inside a deployment. Every mode goes through
service.run_experiment(), so hosted and offline runs share the same pairing,
judging, rating and gate code.
...
from arena.paths import REPO_ROOT as REPO
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "wait-deploy":
        from arena.release import wait_for_commit
    ...
    if args.command == "smoke":
        from arena.release import smoke
