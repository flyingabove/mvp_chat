# .claude/skills/promote-to-prod/arena/service.py
"""One orchestration path for every way an arena run starts.

The hosted CLI stages and the offline local mode (Ollama) both build an
ArenaConfig and call run_experiment(). Stages: play -> judge -> calibrate ->
report. Local-only tooling: never runs inside a deployment.
...
from arena.calibration import CALIBRATION_VERSION, run_calibration
from arena.contracts import (
    BETA, PROD, Budget, ExperimentManifest, ExperimentMode, TargetIdentity, from_dict,
)
from arena.judge import DEFAULT_JUDGE_MODEL, JevPairwiseJudge
from arena.knowledge import load_bundles
from arena.pipeline import JudgePipeline
from arena.players import PERSONAS, PLAYER_PROMPT_VERSION, LLMPlayer
from arena.report import build_arena_report, render_arena_html
from arena.rubric import DEFAULT_RUBRIC
from arena.runner import ArenaRunner, RunSummary, build_pairs
from arena.store import ArtifactStore
from arena.suite import PROFILES, SUITE_VERSION, profile_scenarios
from arena.targets import HostedTargetAdapter
