...
**`gender` / `opening_cast` (opening welcome party):** a `characters[]` entry may declare `gender: "M" | "F"` (kept in `Character.meta`, read via `opening_scene.character_gender()`). `GameState.opening_cast: list[str]` holds the NPC keys staged by `opening_scene.stage_opening_scene()` from the story's `opening.welcome_party`; it and `main_character_id` are persisted by `_serialize_state`. See `SOCIAL_MODE_DESIGN.md` §10.
